...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "giaoHangTpHcm" });
  return {
    title: "Xoài Tứ Quý Giao TP.HCM — 24h Từ Vựa Thạnh Phú | Trái Cây Bến Tre",
    description:
      "Giao xoài Tứ Quý Bến Tre đến TP.HCM trong 24h bằng xe lạnh. Gần vựa nhất — phí ship thấp nhất. Giao tận chợ, siêu thị, nhà hàng. Đặt sỉ: 0327 267 795.",
    keywords: [
...
export default async function GiaoHangTpHcmPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("giaoHangTpHcm");
